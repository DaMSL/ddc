...
def bootstrap (source, N=1000, interval=.95):
  ...
  for v_i in V:
    ...
    probility_est[v_i] = (P_i, delta[round(N*ci_lo)], delta[round(N*ci_hi, 2)])
...
class controlJob(macrothread):
    ...
    # LOAD all new subspaces (?) and values
      ...
      for k, p in pdf_rms.items():
        logging.info('  %s:  u=%0.4f  CI_lo=%0.4f  CI_hi=%0.4f', k, p[0], p[1], p[2])
    ...
    #   Using RMS and PCA, umbrella sample transitions out of state 3
      ...
      #  TODO: MOVE TO CACHE ???
      logging.debug('Checking cache for %d points for reweighting', len(rms_indexlist))
      for i in rms_indexlist:
      ...
      for idx in filelist.keys():
        traj = datareduce.load_trajectory(filelist[idx])
        traj.atom_slice(atomfilter[idx])
        source_traj = traj if source_traj is None else source_traj.join(traj)
      ...
      for i in range(len(rms_proj_pca)):
        ...
        print('POJECTING ', i, '  ---->  ', hcube)
        if hcube not in hcube_keylist:
          hcube_keylist[hcube] = []
        hcube_keylist[hcube].append(i)

      logging.debug('Projected %d points into PCA. Found the following keys:', len(rms_proj_pca))
      for k in hcube_keylist.keys():
        logging.debug('   %s', k)
